...
function Card({ image, title, info, price, discount }: { image: string; title: string; info: string; price: number|string; discount: number|string }) {
  ...
  return (
    ...
    <div className='w-[238px] h-[615px] flex flex-col items-center cursor-pointer relative' onClick={handleCardClick} >
      {isLoading && (
        <div className='absolute inset-0 flex justify-center items-center bg-gray-200 opacity-75 z-10'>
          <div className="animate-spin rounded-full border-t-4 border-blue-500 w-16 h-16"></div>
        </div>
      )}
      <div className='w-full h-[427px] relative hover:scale-105 duration-500'>
